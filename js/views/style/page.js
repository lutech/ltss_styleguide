    ...
    var StylePage = Backbone.View.extend({
        ...
        render_page: function(page) {
            ...
            $(".kalei-sheet-submenu li").on('click', function(ev) {
                $('html, body').animate({
                    scrollTop: $(".kalei-comments h1:contains('"+$(ev.currentTarget).text()+"'), .kalei-comments h2:contains('"+$(ev.currentTarget).text()+"')").offset().top - 100
                }, 400);
            });

            $(window).scroll(function () {
                $(".kalei-documentation").each(function(){
                    if ( that.is_on_screen($(this), 100) ) {
                        $(".kalei-sheet-submenu li").removeClass('active');
                        $(".kalei-sheet-submenu > li:contains('" + $(this).find('.kalei-comments > ul > h1').text() +"'), .kalei-sheet-submenu > ul > li:contains('" + $(this).find('.kalei-comments > h2').text() +"')").addClass('active');
                    }
                });
            });
